 ...
 */
function writeMermaidSidecar(docxPath, markdown) {
  ...
  const sidecarPath = docxPath.replace(/\.docx$/i, '.mermaid-sources.txt');
  const docName = path.basename(docxPath);
  const lines = [
    `# Mermaid sources for ${docName}`,
    `# Exported ${new Date().toISOString()}`,
    `# Total diagrams: ${sources.length}`,
    '',
  ];
...
async function exportToWord({ title, content, author, savePath, mermaidImages, bodyFont, bodySize, saveMermaidSidecar }) {
  const contentParagraphs = await markdownToDocxParagraphs(content, mermaidImages);

  // Body defaults (font/size). docx sizes are in half-points (10pt → 20).
  const defaultRun = {};
  if (bodyFont && typeof bodyFont === 'string' && bodyFont.trim()) {
    defaultRun.font = bodyFont.trim();
  }
  if (bodySize && Number.isFinite(Number(bodySize))) {
    defaultRun.size = Math.round(Number(bodySize) * 2);
  }
  const stylesConfig = Object.keys(defaultRun).length > 0
  ...
  const doc = new Document({
    ...
    ...(stylesConfig ? { styles: stylesConfig } : {}),
    sections: [{
      children: [
        ...headerParagraphs,
        ...contentParagraphs,
  ...
  // Sidecar file with full mermaid sources — defaults to ON since
  // Google Docs can truncate alt text on import.
  if (saveMermaidSidecar !== false) {
    writeMermaidSidecar(savePath, content);
  }
}
...
async function markdownToDocxParagraphs(markdown, mermaidImages) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    switch (token.type) {
      ...
      case 'code': {
        // Mermaid diagrams → embed as image, with source preserved in alt text
        if ((token.lang || '').trim() === 'mermaid' && mermaidImages) {
      ...
      case 'html':
        // Skip HTML tags (style, script, etc.) — not meaningful in Word
        break;
